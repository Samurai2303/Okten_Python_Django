...
def custom_error_handler(exc: Exception, context: dict) -> Response:
    handlers = {
        'JWTException': _jwt_error_handler
    }
...
def _jwt_error_handler(exc: JWTException, context: dict) -> Response:
    return Response(ErrorEnum.JWT.msg, status=ErrorEnum.JWT.status_code)
